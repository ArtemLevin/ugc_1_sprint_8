...
class KafkaBuffer:
    def __init__(self, max_size=None, flush_interval=None):
        ...
        self.flush_interval = flush_interval or settings.kafka.kafka_flush_interval  # Теперь из конфига
            ...
            try:
                ...
                if batch:
                    try:
                        ...
                        logger.warning("Saving messages to DLQ", count=len(batch), error=str(e))
    ...
    async def _flush_queue(self):
        ...
        try:
            ...
            logger.warning("Saving remaining messages to DLQ", count=len(batch), error=str(e))
            await self.dlq_handler.save_messages(batch)
